"""Approved import path for verified NBA performance data."""
...
class UnavailableNBAProvider:
    """Placeholder provider when no NBA data source is configured."""
...
class NBAImportProvider:
    """Load verified NBA data from approved import JSON files."""
    ...
    def _import_file(self) -> Path:
        return self.import_dir / "nba_data.json"
    ...
    def _to_identity(self, player: dict[str, Any]) -> NBAPlayerIdentity:
        sid = str(player["source_player_id"])
        position = player.get("position")
        return NBAPlayerIdentity(
            cs_player_id=cs_nba_player_id(sid),
    ...
    def search_players(self, query: str, limit: int = 10) -> list[NBAPlayerSearchResult]:
        ...
        if len(needle) < 2:
            return []
        results: list[NBAPlayerSearchResult] = []
        for player in self._active_players():
            ...
            results.append(
                NBAPlayerSearchResult(
                    ...
                    source_player_id=sid,
                    player_name=player["player_name"],
                    team=player.get("team") or "NBA",
                    team_id=str(player["team_id"]) if player.get("team_id") else None,
                    position=pos,
                    position_group=map_nba_position(pos),
    ...
    def fetch_recent_games(self, source_player_id: str, limit: int) -> list[NBAGameLogRow]:
        ...
        window = limit if limit > 0 else recent_window_value()
        return valid[:window]
...
def _parse_game_row(raw: dict[str, Any]) -> NBAGameLogRow:
    position = raw.get("position")
    return NBAGameLogRow(
        ...
        position=map_nba_position(position or raw.get("position_group")),
        stats=dict(raw.get("stats") or {}),
    )


def _is_valid_completed_game(row: NBAGameLogRow) -> bool:
...
def get_nba_provider(settings: Settings | None = None) -> NBAPerformanceProvider:
    settings = settings or get_settings()
    import_dir = settings.output_dir / "nba" / "import"
    provider = NBAImportProvider(
