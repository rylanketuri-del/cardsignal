...
from cardchase_ai.alerts import (
    AlertEvent,
    build_daily_digest,
...
from cardchase_ai.models.schemas import (
...
def _build_market_universe(mlb_client: MLBClient, settings) -> list[dict]:
    ...
    # Tier 1: dynamic MLB performance breakouts.
    try:
    ...
    # Tier 2: permanent collector stars / manual tracked players.
...
def _recent_notification_keys(storage: SupabaseStorage, settings) -> set[tuple[str, str, str]]:
    window_hours = max(
        ...
        settings.daily_digest_cooldown_hours,
        1,
    )
